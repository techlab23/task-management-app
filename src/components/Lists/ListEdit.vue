  ...
  <DetailsPopup ref="newListPopup" v-show="this.activeBoard" @popup-toggled="handleNewListPopup">
    ...
    <template v-slot:content>
      <form data-vv-scope="list-form">
        <h4>{{ heading }}</h4>
        <input name="listName" type="text" class="form-control my-1" v-model.trim="listForm.name" v-validate="'required'" data-vv-as="List Name" placeholder="Enter your list name" />
        <small class="text-danger" style="display:block">{{ errors.first('list-form.listName') }}</small>
        <button class="btn btn-sm btn-app mt-2" @click.prevent="startSaveList">Save List</button>
      </form>
    </template>
  </DetailsPopup>
</template>
